...
if(isset($_POST['message']) && !$_SESSION['admin']){
	$_SESSION['comments'][] = filter($_POST['message']);
...
function filter($message){
  $ret = str_ireplace("script","",$message);
  $ret = str_ireplace("img","",$ret);
  $ret = str_ireplace("iframe","",$ret);
  ...
  $ret = str_ireplace("atob","",$ret);
  $ret = str_ireplace("span","",$ret);
  return $ret;
}
?>
  ...
  <body>
    ...
    <form method="POST" action="<?= $_SERVER['SCRIPT_NAME'] ?>">
      <div class="container" style="margin-top: 20px;">
          ...
          <p>Also, the last version was vulnerable so I've had some security, as for now you can't access /admin.php.</p>
          <p>Have a good day :)</p>
